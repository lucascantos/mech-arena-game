    ...
    for (const e of events) {
      if (e.kind === "impact") {
        ...
        if (existing) existing.value += e.amount;
          ...
          const fx: Effect = { kind: "damage", pos: { ...e.pos }, color: UI.damage, value: e.amount, born: now, life: 700 };
  ...
  draw(ctx: CanvasRenderingContext2D): void {
    ...
    for (const fx of this.list) {
      ...
      if (fx.kind === "impact") {
        ...
        ctx.fillText(String(Math.round(fx.value)), fx.pos.x, fx.pos.y - 40 - t * 30);
      }
    }
    ctx.globalAlpha = 1;
  }
